...
<body>
    ...
    if ($hash == null) {
        ...
        $sql = "SELECT * FROM `admin_forgot_password` WHERE `hash` = ?";
        ...
        if ($result->num_rows == 0) {
            ...
            //If all these cconditions are met, display the form 
            if ($status != 'PENDING') {
                echo "<div class='error' style='text-align:center; justify-content:center; align-items:center;'>Sorry, your request is invalid or has expired</div>";
    ...
    ?>
                ...
                <p>Please Enter new pasword below.</p>
                        ...
                        if ($mysqli) {
                            try {
                                ...
                                if ($stmt->affected_rows > 0) {
                                    ...
                                    echo '<p style="text-align:center; justify-content:center; align-items:center;">You have successfully reset your password.</p>';
                                    $submission_successful = true;
                                    echo '</div>';
                                    
                                    $stmt = $mysqli?->prepare("UPDATE `admin_forgot_password` SET `status` = 'EXPIRED' WHERE `hash` = ?");
                                    $stmt->bind_param('s', $hash);
                                    $stmt->execute();
                                ...
                                $log = $ex->getMessage() . ":" . "Error coming from reset-password.php on admin side";
                                error_logger($log);
