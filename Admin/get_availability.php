...
// Defining the maximum bedspaces per room based on hostel type
$bedspaces_per_room = [
    'Single' => 1,
    'Double' => 2,
    'Triple' => 3,
...
// Fetch all rooms and bedspaces for the selected hostel
...
$available_rooms = [];
for ($room = 1; $room <= 15; $room++) { // Because there is 15 rooms total
    $occupied_bedspaces = [];
    
    // Find if this room is in the results
    ...
    if (!empty($available_bedspaces)) {
        $available_rooms[] = [
            'room_number' => $room,
            'bedspace_number' => array_values($available_bedspaces) // Re-index array to be safe
        ];
    }
}
