    ...
    if ($mysqli) {
        try {
            ...
            if (isset($_SESSION['id'])) {
                ...
                $sql = "SELECT * FROM complaint_resolution WHERE student_id = ? AND is_read = 0 ORDER BY `date_added` LIMIT $records_per_page OFFSET $offset";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<p>" . $row['complaint_solution'] . "</p>";
                        echo "<span class='timestamp'>" . $row['date_added'] . "</span>";
                        echo '</div>';
                    }

                    
                    echo "<button class='mark-all-read-btn'><a href='mark_conflict_resolution_read.php'>Mark all as read</a></button>";
            ...
            $log = $ex->getMessage() . ":" . "This is from the complaint_resolution.php file on the client side";
            error_logger($log);
