...
if (isset($_POST['submit']) && $mysqli) {
    ...
    try {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if (password_verify($password, $hash)) {
                        $message = '<div class="success">You have successfully logged in!</div>';
                        $_SESSION['email'] = $email;
                        $_SESSION['id'] = $row['id']; 
                        $_SESSION['student_name'] = $row['student_name'];
                        $_SESSION['hostel'] = $row['hostel'];
                        $_SESSION['bedspace_number'] = $row['bedspace_number'];
        ...
        $log = $ex->getMessage() . ":" . "This is from the login.php file on the client side";
        error_logger($log);
...
<body>
    ...
    <p style="font-size:1em">Don't have an Account?
